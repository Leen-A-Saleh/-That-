// Data comes from PHP via window.THERAPISTS_DATA

const ITEMS_PER_PAGE = 6;

let therapistsData = (window.THERAPISTS_DATA || []).map((t) => ({
  id: parseInt(t.therapist_id),
  name: t.name,
  email: t.email,
  specialty: t.specialization,
  experience: parseInt(t.experience_years) || 0,
  cases: parseInt(t.total_cases) || 0,
  rating: parseFloat(t.rating) || 0,
  status: t.status, // DB value: AVAILABLE | BUSY | ON_LEAVE
}));
...
function getStatusClass(status) {
  ...
  if (status === "ON_LEAVE")  return "vacation";
  return "available";
}
...
function openViewModal(id) {
  ...
  document.getElementById("vm-experience").textContent = t.experience + " سنوات";
  document.getElementById("vm-cases").innerHTML  = `<span class="cases-badge">${t.cases}</span>`;
  document.getElementById("vm-rating").innerHTML = `<i class="fa fa-star"></i> ${t.rating}`;
  document.getElementById("vm-status").innerHTML =
    `<span class="status-badge ${getStatusClass(t.status)}">${getStatusLabel(t.status)}</span>`;
      ...
      <tr>
        <td colspan="7">
          <div class="empty-state">
            ...
            <p>لا يوجد أخصائيون مطابقون للبحث</p>
          </div>
        </td>
      </tr>`;
    return;
          ...
          <div class="therapist-card-info">
            ...
            <div>
              ...
              <div class="therapist-email">
                <i class="fa fa-envelope" style="font-size:11px;color:#9ca3af;"></i>${t.email}
              </div>
            </div>
          </div>
          <span class="status-badge ${getStatusClass(t.status)}">${getStatusLabel(t.status)}</span>
...
function openModal(mode, id = null) {
  ...
  if (mode === "add") {
    ["fieldName", "fieldEmail", "fieldSpecialty", "fieldExperience", "fieldRating"].forEach(
...
function saveTherapist() {
  ...
  const experience  = parseInt(document.getElementById("fieldExperience").value) || 0;
  ...
  let hasError = false;
  if (!name)      { markFieldError("fieldName",      "الاسم الكامل مطلوب");     hasError = true; }
  if (!email)     { markFieldError("fieldEmail",     "البريد الإلكتروني مطلوب"); hasError = true; }
  if (!specialty) { markFieldError("fieldSpecialty", "يرجى اختيار التخصص");     hasError = true; }
  ...
  // Build FormData and POST to therapists.php
  const formData = new FormData();
  formData.append("action",           "save");
  formData.append("id",               editingId || 0);
...
// Event listeners
document.getElementById("addTherapistBtn").addEventListener("click", () => openModal("add"));
...
document.getElementById("nextBtn").addEventListener("click", () => {
  if (currentPage < Math.ceil(filteredData.length / ITEMS_PER_PAGE)) { currentPage++; renderAll(); }
});
